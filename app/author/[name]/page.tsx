...
export async function generateMetadata({ params }: { params: { name: string } }): Promise<Metadata> {
  ...
  return {
    title: `${name} — stories by this author`,
    description: `All stories written by ${name} on Ghibli Gazette — anime & manga news hub.`
  }
}
...
export default async function AuthorPage({ params }: { params: { name: string } }) {
  ...
  const posts = (await getAllPosts()).filter(
    (p) => p.published && p.author.trim().toLowerCase() === name.trim().toLowerCase()
  )
